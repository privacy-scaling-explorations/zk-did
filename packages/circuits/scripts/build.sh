...
function build_circuit() {
    circom ./src/main.circom  --r1cs --wasm -o "$BUILD_DIR" -l ./node_modules -l ../../node_modules
}
...
function dev_trusted_setup() {
	...
	node $SNARKJS_PATH groth16 setup "$BUILD_DIR"/main.r1cs "$PTAU_PATH" "$PARTIAL_ZKEYS_DIR"/circuit_0000.zkey
    ...
    cp "$JS_BUILD_DIR"/main.wasm "$ARTIFACTS_DIR"
